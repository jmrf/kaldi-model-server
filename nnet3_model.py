...
class ASRRedisClient:
    def __init__(
        self, red, server="localhost", channel="asr", record_message_history=False
    ):
...
def print_devices(paudio):
    ...
    for i in range(0, numdevices):
        if (
            paudio.get_device_info_by_host_api_device_index(0, i).get(
                "maxInputChannels"
            )
            > 0
        ):
            print(
                ...
                i,
                " - ",
                paudio.get_device_info_by_host_api_device_index(0, i).get("name"),
            )
